...
def test_collect(package_complex):
        ...
        "targets": {
            os.path.join(root, "build", f"test{EXT_SUFFIX}"): {
                "destination": os.path.join(
                    "{py_platlib}", "complex", f"test{EXT_SUFFIX}"
                ),
                "tag": "runtime",
            },
        },
    ...
    assert tree["complex"]["__init__.py"] == os.path.join(
        root, "complex", "__init__.py"
    )
    assert tree["complex"][f"test{EXT_SUFFIX}"] == os.path.join(
        root, "build", f"test{EXT_SUFFIX}"
    )
    assert tree["complex"]["more"]["__init__.py"] == os.path.join(
        root, "complex", "more", "__init__.py"
    )
...
def test_mesonpy_meta_finder(package_complex, tmp_build_path):
    ...
    try:
        ...
        assert complex.__spec__.origin == os.fspath(
            package_complex / "complex/__init__.py"
        )
        assert complex.__file__ == os.fspath(package_complex / "complex/__init__.py")
        import complex.test

        assert complex.test.__spec__.origin == os.fspath(
            build_path / f"test{EXT_SUFFIX}"
        )
        assert complex.test.answer() == 42
        import complex.namespace.foo

        assert complex.namespace.foo.__spec__.origin == os.fspath(
            package_complex / "complex/namespace/foo.py"
        )
...
def test_mesonpy_traversable():
    ...
    assert {x.name for x in traversable.iterdir()} == {
        "__init__.py",
        "src.py",
        "data.txt",
        "nested",
    ...
    assert {x.name for x in nested.iterdir()} == {
        "__init__.py",
        "some.py",
        "generated.txt",
    }
...
def test_resources(tmp_path):
    ...
    finder = _editable.MesonpyMetaFinder(
        {"simple"}, os.fspath(tmp_path / "build"), ["ninja"]
    )
...
@pytest.mark.skipif(
    sys.version_info < (3, 9), reason="importlib.resources not available"
)
def test_importlib_resources(tmp_path):
    ...
    try:
        ...
        assert text == "ABC"
        assert (
            importlib.resources.files(simple).joinpath("data.txt").read_text().rstrip()
            == "ABC"
        )
    finally:
        # remove finder from the meta path
