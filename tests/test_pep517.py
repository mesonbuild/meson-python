...
@pytest.mark.parametrize(
    "system_patchelf", ["patchelf", None], ids=["patchelf", "nopatchelf"]
)
@pytest.mark.parametrize(
...
def test_get_requires_for_build_wheel(monkeypatch, package, system_patchelf, ninja):
    ...
    ninja_available = ninja is not None and [int(x) for x in ninja.split(".")] >= [
        1,
        8,
        2,
    ]
...
def test_validate_config_settings_repeated():
    with pytest.raises(
        mesonpy.ConfigError, match='Only one value for "builddir" can be specified'
    ):
