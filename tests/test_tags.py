...
def test_macos_platform_tag(monkeypatch):
    for minor in range(9, 16):
        ...
        assert (
            next(packaging.tags.mac_platforms((10, minor)))
            == mesonpy._tags.get_platform_tag()
        )
...
def wheel_builder_test_factory(monkeypatch, content):
    ...
    return mesonpy._WheelBuilder(
        None, None, pathlib.Path(), pathlib.Path(), pathlib.Path()
    )
...
@pytest.mark.skipif(
    not ABI3SUFFIX, reason="Stable ABI not supported by Python interpreter"
)
