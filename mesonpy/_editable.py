...
class SourceFileLoader(importlib.machinery.SourceFileLoader):
    ...
    def set_data(
        self, path: Union[bytes, str], data: bytes, *, _mode: int = ...
    ) -> None:
...
def build_module_spec(
    cls: type, name: str, path: str, tree: Optional[Node]
...
class Node(NodeBase):
    ...
    def __setitem__(
        self, key: Union[str, Tuple[str, ...]], value: Union[Node, str]
    ) -> None:
        node = self
            ...
            if path.parts[0] in {"{py_platlib}", "{py_purelib}"}:
                if key == "install_subdirs" and os.path.isdir(src):
                    for entry in walk(src):
                        tree[(*path.parts[1:], *entry.parts)] = os.path.join(
                            src, *entry.parts
                        )
                else:
...
class MesonpyMetaFinder(importlib.abc.MetaPathFinder):
    def __init__(
        self, names: Set[str], path: str, cmd: List[str], verbose: bool = False
    ):
    ...
    def rebuild(self) -> Node:
        ...
        subprocess.run(
            self._build_cmd, cwd=self._build_path, env=env, stdout=stdout, check=True
        )

        install_plan_path = os.path.join(
            self._build_path, "meson-info", "intro-install_plan.json"
        )
